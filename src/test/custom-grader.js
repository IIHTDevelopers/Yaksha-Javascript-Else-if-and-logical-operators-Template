...
// Function to check if ternary operator is used
function checkTernaryOperatorUsage(ast) {
    let result = 'Passed';
    let feedback = [];
    let ternaryUsed = false;

    ast.body.forEach((node) => {
        // Check for ternary operator in 'ExpressionStatement' and 'VariableDeclaration'
        if (node.type === 'ExpressionStatement' && node.expression.type === 'ConditionalExpression') {
            ternaryUsed = true;
        }

        // Check if ternary operator is used in variable declarations (e.g., var result = condition ? true : false)
    ...
    if (!ternaryUsed) {
        result = 'Failed';
        feedback.push("You must use the ternary operator (? :) for conditional expressions.");
    }

    // Detailed logging of the check
    console.log(`\x1b[33mChecking ternary operator usage\x1b[0m`);
...
function gradeAssignment() {
    ...
    testResults.testCaseResults[GUID] = checkTernaryOperatorUsage(ast);
